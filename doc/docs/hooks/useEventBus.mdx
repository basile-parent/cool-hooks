...
```tsx
const ComponentUsingEventBus = ({prop1}: MyComponentProps) => {
    ...
    return (
        <button onClick={() => emit("channel-1", "Some message")}>Send message</button>
    )
}
```
...
|---------------|---------------------------------------------------|--------------------------------------------------------------------------------------------------|
| *subscribe*   | (channel: string, callback: CallbackType) => void | Subscribe to a channel an call the _callback_ function whenever a message is emitted through it. |
| *unsubscribe* | (channel: string, callback: CallbackType) => void | Unsubscribe to a channel. The _callback_ function must be the same used for the subscription.    |
| *emit*        | (channel: string, message: any) => void           | Emit a message on a specific channel                                                             |                                                             
